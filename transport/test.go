...
func (suite *TransportTestSuite) TestSendReceive() {
	...
	go func() {
		...
		case req := <-inboundChan:
			...
			rsp.SetOriginService(req.OriginService())
			rsp.SetOriginEndpoint(req.OriginEndpoint())
	...
	req.SetEndpoint("foo")
	req.SetOriginService("testsuite")
	req.SetOriginEndpoint("testsendreceive")
	...
	suite.Assert().Equal(req.OriginService(), rsp.OriginService())
	suite.Assert().Equal(req.OriginEndpoint(), rsp.OriginEndpoint())
...
func (suite *TransportTestSuite) TestSendReceiveParallel() {
	...
	work := func(i int) {
		...
		req.SetEndpoint("foo")
		req.SetOriginService("testsuite")
		req.SetOriginEndpoint("testsendreceiveparallel")
